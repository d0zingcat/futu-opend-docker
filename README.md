...
the Goldmachine checkout and never publishes OpenD ports.

The Compose project owns these resources:

- `goldmachine-futu` — an internal Docker network shared with Goldmachine;
...
key, so the matching Goldmachine client key remains valid on every redeploy.
...
```dotenv
FUTU_ACCOUNT_ID=
FUTU_OPEND_NETWORK=goldmachine-futu
FUTU_OPEND_STATE_VOLUME=futu-opend-state
FUTU_OPEND_SECRETS_VOLUME=futu-opend-secrets
```

Goldmachine must use the same `FUTU_OPEND_NETWORK` and
...
### Migrate an existing Goldmachine sidecar

1. Record and back up the current OpenD state volume. In the running Dokploy
   stack, its usual name is `<dokploy-project>_goldmachine-futu-state`; inspect
   the actual volume name before continuing.
...
3. In this service's Dokploy Environment, set `FUTU_OPEND_STATE_VOLUME` to the
   ...
   `..._goldmachine-futu-secrets` volume name. Deploy this service. The init
   job detects and keeps the existing RSA key; the login/device cache remains
   in place.
4. Deploy the Goldmachine Compose change with the same secrets-volume name and
   `FUTU_OPEND_NETWORK=goldmachine-futu`. Goldmachine needs no OpenD password.
